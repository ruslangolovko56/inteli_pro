...
(function () {
  ...
  // Width of a node.
  //
  quizmap.nodeWidth = 150;

  //
  // Height of a node.
  //
  quizmap.nodeHeight = 100;

  //
  // Minimum width of a node.
  //
  quizmap.nodeMinWidth = 150;

  //
  // left padding of a node.
  //
  quizmap.nodeLeftPadding = 40;

  //
  // Amount of space reserved for displaying the node's name.
  //
  quizmap.nodeNameHeight = 40;

  //
  // Height of a connector in a node.
  //
  quizmap.connectorHeight = 35;

  //
  // Width of a connector in a node.
  //
  quizmap.connectorWidth = 35;

  //
  // Height of map view.
  //
  quizmap.mapHeight = 690;

  //
  // Width of map view.
  //
  quizmap.mapWidth = 1100;

  //
  // Increasable height size.
  //
  quizmap.incSize = 400;
  ...
  // Map container id.
  //
  quizmap.mapContainerId = '';

  //
  // Compute the X coordinate of a connector, given its index.
  //
  quizmap.computeConnectorX = function (connectorIndex, width, count) {
    var paddingLeft = parseInt((width - (count - 1) * quizmap.connectorWidth) / 2);
    return paddingLeft + (connectorIndex * quizmap.connectorWidth);
  }

  //
  // Compute the position of a connector in the graph.
  //
  quizmap.computeConnectorPos = function (node, connectorIndex, inputConnector) {
    return {
      x: node.x() + quizmap.computeConnectorX(connectorIndex, node.width(), inputConnector ? node.data.inputConnectors.length : node.data.outputConnectors.length),
      y: node.y() + (inputConnector ? 0 : node.height()),
    };
  };
  ...
  //
  quizmap.ConnectorViewModel = function (connectorDataModel, x, y, parentNode) {
    ...
    if (connectorDataModels) {
      for (var i = 0; i < connectorDataModels.length; ++i) {
        var connectorViewModel =
              new quizmap.ConnectorViewModel(connectorDataModels[i], quizmap.computeConnectorX(i, parentNode.width(), connectorDataModels.length), y, parentNode);
  ...
  //
  quizmap.NodeViewModel = function (nodeDataModel) {
    ...
    this.width = function () {
      var numConnectors =
            Math.max(
              this.data.inputConnectors.length,
              this.data.outputConnectors.length);
      return Math.max(quizmap.nodeLeftPadding * 2 + (numConnectors - 1) * quizmap.connectorWidth, quizmap.nodeMinWidth);
    ...
    this.height = function () {
      ...
      var nRowCharacter = Math.floor(width / this._fontwidth);
      var nRow = Math.ceil(name.length / nRowCharacter);

      if (this.data.meta && this.data.meta.exitUrl) {
        nRow += Math.ceil(this.data.meta.exitUrl.length / nRowCharacter);
      }

      if (this.data.meta && this.data.meta.list && this.data.meta.list.listName) {
        nRow += Math.ceil(this.data.meta.list.listName.length / nRowCharacter);
      }

      return Math.max(quizmap.nodeHeight, nRow * 20 + 50);
    }

    this.inputConnectors = createConnectorsViewModel(this.data.inputConnectors, 0, this);
    this.outputConnectors = createConnectorsViewModel(this.data.outputConnectors, this.height(), this);
    ...
    this._addConnector = function (connectorDataModel, y, connectorsDataModel, connectorsViewModel) {
      var connectorViewModel =
            new quizmap.ConnectorViewModel(connectorDataModel, quizmap.computeConnectorX(connectorsViewModel.length, this.width(), connectorsDataModel.length), y, this);
    ...
    this.addOutputConnector = function (connectorDataModel) {
      ...
      this._addConnector(connectorDataModel, quizmap.nodeHeight, this.data.outputConnectors, this.outputConnectors);
    };

    //
    // Split name into multiline to display in node rect.
    //
    this.multilineString = function (str) {
      var name = str || "";
      var nameArr = [];
      var width = this.width();
      var nRowCharacter = Math.floor(width / this._fontwidth);

      if (name.length > nRowCharacter) {
        var nRow = Math.ceil(name.length / nRowCharacter);
        for (var i = 0; i < nRow; i++) {
          nameArr.push(name.substring(i * nRowCharacter, Math.min((i + 1) * nRowCharacter, name.length)));
        }
      } else {
        nameArr.push(name);
      }
    ...
    // Set multiline name.
    //
    this.name = this.multilineString(this.data.name);

    if (this.data.type === 'esp') {
      this.listName = (this.data.meta && this.data.meta.list) ? this.multilineString(this.data.meta.list.listName) : '';
    }

    if (this.data.type === 'resultsPage' || this.data.type === 'form') {
      this.exitUrl = (this.data.meta) ? this.multilineString(this.data.meta.exitUrl) : '';
    }
  ...
  var createNodesViewModel = function (nodesDataModel) {
    ...
    if (nodesDataModel) {
      for (var i = 0; i < nodesDataModel.length; ++i) {
        nodesViewModel.push(new quizmap.NodeViewModel(nodesDataModel[i]));
      }
    }
  ...
  //
  quizmap.ConnectionViewModel = function (connectionDataModel, sourceConnector, destConnector) {
    ...
    this.sourceTangentX = function () {
      return quizmap.computeConnectionSourceTangentX(this.sourceCoord(), this.destCoord());
    };

    this.sourceTangentY = function () {
      return quizmap.computeConnectionSourceTangentY(this.sourceCoord(), this.destCoord());
    };
    ...
    this.destTangentX = function () {
      return quizmap.computeConnectionDestTangentX(this.sourceCoord(), this.destCoord());
    };

    this.destTangentY = function () {
      return quizmap.computeConnectionDestTangentY(this.sourceCoord(), this.destCoord());
    };
  ...
  //
  quizmap.computeConnectionSourceTangentX = function (pt1, pt2) {
  ...
  // Compute the tangent for the bezier curve.
  //
  quizmap.computeConnectionSourceTangentY = function (pt1, pt2) {
  ...
  // Compute the tangent for the bezier curve.
  //
  quizmap.computeConnectionSourceTangent = function (pt1, pt2) {
    return {
      x: quizmap.computeConnectionSourceTangentX(pt1, pt2),
      y: quizmap.computeConnectionSourceTangentY(pt1, pt2),
    };
  };
  ...
  // Compute the tangent for the bezier curve.
  //
  quizmap.computeConnectionDestTangentX = function (pt1, pt2) {
  ...
  // Compute the tangent for the bezier curve.
  //
  quizmap.computeConnectionDestTangentY = function (pt1, pt2) {
  ...
  // Compute the tangent for the bezier curve.
  //
  quizmap.computeConnectionDestTangent = function (pt1, pt2) {
    return {
      x: quizmap.computeConnectionDestTangentX(pt1, pt2),
      y: quizmap.computeConnectionDestTangentY(pt1, pt2)
    };
  };
  ...
  //
  quizmap.ChartViewModel = function (chartDataModel) {
    ...
    this._createConnectionViewModel = function (connectionDataModel) {
      ...
      return new quizmap.ConnectionViewModel(connectionDataModel, sourceConnector, destConnector);
    }
    ...
    this.createNewConnection = function (startConnector, endConnector) {
      ...
      var connectionViewModel = new quizmap.ConnectionViewModel(connectionDataModel, outputConnector, inputConnector);
    ...
    this.addNode = function (nodeDataModel) {
      ...
      //
      this.nodes.push(new quizmap.NodeViewModel(nodeDataModel));
    }
